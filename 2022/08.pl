...
sub day08_1 {
    ...
    my @seen_array = ();
    foreach my $i (0..$data_len-1) {
        push(@seen_array, [(0)x($element_len-1)]);
    }

    # left to right, right to left
    foreach my $row (0..$data_len-1) {
        my ($tallest_tree_left, $tallest_tree_right) = (-1, -1);
        foreach my $col (0..$element_len-1) {
...
sub day08_2 {
    ...
    foreach my $row (1..$data_len-2) {
        foreach my $col (1..$element_len-2) {
